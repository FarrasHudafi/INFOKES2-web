    ...
    <div className="mb-10 items-center px-5 flex flex-col gap-2 ">
      ...
      <div className='grid grid-cols-3 mt-5 md:grid-cols-4 lg:grid-cols-6 '>
        {categoryList.length>0? categoryList.map((item, index) => index<6 && (
          <Link href={'/search/'+item.Name} key={index} className='flex flex-col gap-2 text-center items-center p-5 bg-blue-50 m-2 rounded-lg cursor-pointer hover:scale-110 transition-all ease-in-out'>
                  <Image 
                  src={item.Icon?.url}
                  alt='icon'
                  width={40}
